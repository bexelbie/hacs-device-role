...
async def test_fake_device_entities_exist(ha_client):
    """Both integrations load and fake_device entities are reachable."""
    state = await ha_client.get_state("sensor.test_plug_temperature")
    assert state is not None
    assert state["state"] != "unavailable"


@pytest.mark.usefixtures("ha_bootstrap")
async def test_role_mirrors_temperature(ha_client):
...
@pytest.mark.usefixtures("ha_bootstrap")
async def test_role_mirrors_switch(ha_client):
